...
$s = 0;
$get_futbol_bulten =  json_decode(misli_canli("SOCCER"), true);
            ...
            <?php foreach($match_data[$key2] as $key=>$match_value):
                ...
                <div class="nPMatch" data-name="<?=$match_value['name']?>" data-date="<?=$key2?>">
                    <ul class="fDatas">
                        ...
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['ms_x_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_x']?>', 'MS X', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['ms_x']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['ms_2_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_2']?>', 'MS 2', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['ms_2']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['ig_1_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ig_1']?>', 'IG 1', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['ig_1']?></li>
                        <li class="olmaz tCenter <?=cs_status($match_value['odds']['ig_x_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ig_x']?>', 'İG X', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['ig_x']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['ig_2_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ig_2']?>', 'İG 2', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['ig_2']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['tg_1_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_1a']?>', 'MS A/Ü 1/A', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tg_1']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['tg_2_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_0a']?>', 'MS A/Ü 0/A', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tg_2']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['tg_3_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_2a']?>', 'MS A/Ü 2/A', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tg_3']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['tg_4_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_1u']?>', 'MS A/Ü 1/Ü', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tg_4']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['tc_1_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_0u']?>', 'MS A/Ü 0/Ü', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tc_1']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['tc_2_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['ms_au_2u']?>', 'MS A/Ü 2/Ü', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['tc_2']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['au_25_a_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['au_25_a']?>', 'AÜ 2,5 Alt', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['au_25_a']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['au_25_u_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['au_25_u']?>', 'AÜ 2,5 Üst', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['au_25_u']?></li>
                        <li class="bsi tCenter <?=cs_status($match_value['odds']['kg_var_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['kg_var']?>', 'KG VAR', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['kg_var']?></li>
                        <li class="bsi tCenter bRightBlack <?=cs_status($match_value['odds']['kg_yok_cs'])?>" onclick="addOdd('<?=$match_value['name']?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$match_value['odds']['kg_yok']?>', 'KG YOK', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><i></i><?=$match_value['odds']['kg_yok']?></li>
                    ...
                    <div class="tumOranlar">
                        ...
                        foreach($match_value_data[$key2][$key] as $key_odds_data_r=>$value_odds_data_r): ?>
                            <div class="oranItem">
                                <div class="text"><span><?=$value_odds_data_r['MN']?></span></div>
                                <ul>
                                    <li class="mbs <?=mbs_to_string($value_odds_data_r['OL'][0]['MBC'])?>"><span>MBS</span><i><?=$value_odds_data_r['OL'][0]['MBC']?></i></li>
                                    <?php foreach($value_odds_data_r['OL'] as $key_odds_data=>$value_odds_data): ?>
                                        <li onclick="addOdd('<?=$match_data?>','<?=$match_code[$key2][$key]?>', '<?=$match_mbs[$key2][$key]?>', '<?=$value_odds_data['O']?>', '<?=$value_odds_data['MN']?> <?=$value_odds_data['CSN']?>', '<?=$match_lig[$key2][$key]?>','<?=strtotime($key2)?>', '<?=$match_hour[$key2][$key]?>')"><span><?=$value_odds_data['CSN']?></span><i><?=$value_odds_data['O']?></i></li>
                                    <?php endforeach; ?>
                                </ul>
                            </div>
...
<style>
    .firstHeadline{display: none;}
    .sHeader{height: 52px !important;}
</style>
